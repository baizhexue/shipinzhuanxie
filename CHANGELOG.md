...
- Telegram 视频发送选择改为紧跟处理方式选择出现，快速转写、高精度转写和只下载视频均使用同一套交互。
- 转写任务会保留“是否发送原视频”的预先选择；选择发送时，任务完成后同时回发转写稿和原视频。
- 视频选择绑定原 chat、发起用户和任务且只能使用一次，补齐跨用户、重复点击与过期选择保护。

### 优化

- 保留升级前已发出的下载完成确认按钮兼容性；上传失败仍可用绑定原任务的按钮重试。
- README 更新为“处理方式 → 是否发送原视频 → 总结方式（如适用）→ 执行”的实际流程。

## [0.10.0] - 2026-08-02

### 功能新增

- Telegram “只下载视频”模式在下载完成后显示“发送原视频 / 不发送”Inline Keyboard，只有用户明确确认后才上传原视频。
- 发送确认绑定原 chat、发起用户、下载任务和确认消息，并持久化处理发送中、重复点击、过期与文件已清理状态。

### 修复

- 在上传前主动检查 Telegram 官方云端 Bot API 的 50 MB 文件上限，超限或发送失败时给出明确提示，不再产生静默失败。
- 保持原有快速转写、高精度转写、白名单和转写文件回传流程不变。
- 修复部分平台首次启动后 30 秒内不执行卡住任务清理，以及 macOS 路径别名导致 Deno PATH 探测测试失真的问题。

### 优化

- Telegram 状态文件改为原子替换写入，降低下载线程与回调并发更新造成状态损坏的风险。
- README 和 `.env.example` 补充确认流程、上限、失败策略、配置项与升级方法。
