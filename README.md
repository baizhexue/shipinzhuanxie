...
- 选择处理方式后，立即在同一条消息中选择“发送原视频”或“不发送”；转写模式如已配置 DeepSeek，再继续选择总结方式
- 转写模式执行 `download + transcribe`，完成后回发转写文本；如果预先选择“发送原视频”，还会把原视频回发到发起请求的同一会话
- “只下载视频”不会进入转写；同样只有预先选择“发送原视频”才会在下载完成后上传到 Telegram
- 视频选择绑定 chat、user 和任务且只能使用一次；过期、重复点击、跨会话或跨用户点击都不会重复或错发视频
- 处理中会按阶段回推任务进度
...
- 文件已被清理、选择超时、上传失败时都会在原会话明确提示；上传失败会提供按钮重试
...
升级前建议记录当前提交 `git rev-parse HEAD` 作为回滚点。机器人状态文件兼容旧格式；新版本会增加待发送选择记录，并继续识别升级前已经发出的旧确认按钮。请不要把该运行时文件提交到 Git。
